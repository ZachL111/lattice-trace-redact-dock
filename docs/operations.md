...
`lattice-trace-redact-dock` is centered on observability work in Scala. The technical target is: Package a Scala local lab for redact analysis with bounded scenario files, conflict explanations, and documented operating limits.
...
The score is deterministic. The current threshold is 155. Risk has a penalty of 7, latency has a penalty of 3, and weight contributes a bonus of 3.
